...
function findDuplicateKeys(children: ExtendedNode[]): ExtendedNode[] {
  const duplicates = [];
  for (let i = 0; i < children.length; i++) {
    const current: any = children[i];
    if (current.type === "property") {
      for (let j = 0; j < children.length; j++) {
        const other: any = children[j];
        if (
          other.type === "property" &&
...
export function parseTree(
  ...
  let currentParent: ExtendedNode = {
    type: "array",
    offset: -1,
    length: -1,
    children: [],
  ...
  function onValue(valueNode: ExtendedNode): ExtendedNode {
